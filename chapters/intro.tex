...
In this paper, we explored two ideas: ``Combining Convolution and Recursive Neural Networks'' (the main idea) and ``Trainsfer Learning From Large Review Dataset'' (the support idea).
\begin{description}
	...
	By replacing the standard Glove\footnote{Common Crawl (840B tokens, 2.2M vocab, cased, 300d vectors, 2.03 GB download) publicly available at \url{https://nlp.stanford.edu/projects/glove/}} with Glove Amazon, many models are able to gained large improvements when evaluated on Stanford Sentiment Treebank.
