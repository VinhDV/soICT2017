...
\begin{itemize}
	\item CNN-Tree-LSTM is able to outperforms Constituency Tree-LSTM~\cite{treeLSTM}, CNN-multichannel\cite{KimCNN}.
	This result supports our hypothesis on the benefits of combining convolution layers with Tree-LSTM.
	\item CNN-LSTM outperforms LSTM~\cite{treeLSTM} and CNN-multichannel\cite{KimCNN} as well as other models in Block A. 
	This result supports our hypothesis on the drawback of using k-max-pooling layer in Block A's models.
	\item Due to using similar architects, CNN-LSTM was comparable with CNN-GRU-word2vec and CNN-LSTM-word2vec~\cite{cnn-rnn}.
	The max pooling layer in CNN-LSTM-word2vec might help it to better generalize compares to our CNN-LSTM.
	\item In all our experiments, Glove Amazon is better than the standard Glove.
...
The fact that CNN-Tree-LSTM is able to outperforms Constituency Tree-LSTM~\cite{treeLSTM}, CNN-multichannel\cite{KimCNN} supports our hypothesis on the benefits of combining convolution layers with Tree-LSTM.
Similarly, CNN-LSTM outperforms LSTM~\cite{treeLSTM} and CNN-multichannel\cite{KimCNN} by a good margin. 
