...
For training a Recurrent Neural Network, any phrase spanned by a labeled node is treated as a training sample. Implementations of our models are available at \url{https://github.com/ttpro1995/Tree_CNN_LSTM}.
...
Similar to our approach, Radford st al.~\cite{mlstm} did transfer learning from large review dataset (which is also Amazon Reviews dataset) to improve the performance of their models on Stanford Sentiment Treebank.
...
After that, vector representations of all sentences in Stanford Sentiment Treebank are computed using the trained byte-level multiplicative LSTM.
The vector representations along with their corresponding labels are used to train a logistic regression classifier~\cite{mlstm}.
